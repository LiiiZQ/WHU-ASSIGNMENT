		...
		if (!cmmCompiler) {
			...
			try {
				...
				for (Token token : displayTokens) {
					Style s = null;
					if (keywords.contains(token.getValue()) && token.getType() == TYPE.KEYW) {
						s = styleContext.getStyle("keywords");
					...
					} else if (token.getType() == TYPE.COMMENT) {
						s = styleContext.getStyle("comment");
					} else if (token.getValue().equals("/*")) {
						s = styleContext.getStyle("comment");
					} else if (token.getValue().equals("*/")) {
		...
		for (Token token : displayTokens) {
			if (token.getType() == TYPE.ERROR) {
				if (token.getLineNo() != 1) {
					drawRedWaveLine(g, token.getColumn() * charWidth, (token
							.getLineNo() - 1)
							* LINE_LENGTH_Y + FIRST_POSITION_Y, sectionX,
							sectionX, token.getValue().length());
				} else {
					drawRedWaveLine(g, token.getColumn() * charWidth,
